	...
	if(USART3_RX_STA&0X8000)			//接收到一次数据了
		{
			...
			/*for(t=0;t<3;t++)
			{
				data_char[t]=USART3_RX_BUF[t];
			}
			data_char[3]=0;*/
			
			for(t=3;t<len;t++)
			{
				data_num[t-3]=USART3_RX_BUF[t];
			}
			data_num[len-3]=0;
			...
			if(len>2)				
			{
				if(strncmp((const char*)USART3_RX_BUF,"pan",3)==0){steer2_detect=steer2+atoi(data_num);flag_detect=1;}		//steer2
				if(strncmp((const char*)USART3_RX_BUF,"tit",3)==0&&flag_detect==1){steer1=steer1+atoi(data_num);delay_ms(10);steer2=steer2_detect;delay_ms(10);flag_detect=0;}		//steer1
				if(strncmp((const char*)USART3_RX_BUF,"kai",3)==0){LED=0;beep=1;}
				if(strncmp((const char*)USART3_RX_BUF,"off",3)==0){LED=1;beep=0;}
			}

			//if(len==1) 		//控制DS1检测
			//if(len>2)				
			//{
			//	if(strcmp((const char*)data_char,"pan")==0){beep=1;steer2=atoi(data_num);}		//steer2
			//	if(strcmp((const char*)data_char,"tit")==0){beep=1;steer1=atoi(data_num);}		//steer1
			//}
			u3_printf("\r\n\r\n");//插入换行
			USART3_RX_STA=0;	 
		}	
