//接线：接板载的oled接口，为软件SPI模拟
//分辨率为128*64

//OLED_Show显示函数，在这个函数里面更改，从而更改显示的内容，再在主函数中调用
//OLED_Display_On开启OLED
//OLED_Display_Off关闭OLED
//OLED_Refresh_Gram更新oled函数		   				   		    
//OLED_Clear清除屏幕信息函数
//OLED_DrawPoint
//OLED_ShowChar显示字符函数
//OLED_ShowNumber显示数字函数
//OLED_ShowString显示字符串函数
//OLED_ShowCHinese显示汉字函数	
//OLED_Set_Pos
...
#include "oled.h"
#include "stdlib.h"		
#include "oledfont.h" 	//该头文件中定义了字符和汉字的取模数组	 
#include "delay.h"
#include "encoder.h"

extern int decoder_left,decoder_right;     //左右编码器的脉冲计数
extern float pitch,roll,yaw; 				//欧拉角
extern int flag;		//循迹检测的结果变量
void OLED_Show()	//注意如果放在主函数的死循环中，会重复执行这个函数
{		

		decoder_left=Read_Encoder(2);
		decoder_right=Read_Encoder(3);
		//flag,pitch,roll,yaw;的值都与全局变量值一致
	
		OLED_Display_On();
		OLED_ShowString(50,0,"flag:");
		OLED_ShowNumber(95,0,flag,3,16);
	
		OLED_ShowString(0,15," decoder_l:");
		if(decoder_left>=0)
		{
			OLED_ShowString(90,15,"+");
			OLED_ShowNumber(95,15,decoder_left,3,16);
		}else{
			OLED_ShowString(90,15,"-");
			OLED_ShowNumber(95,15,decoder_left,3,16);
		}
		
		OLED_ShowString(0,30," decoder_r:");
		if(decoder_right<=0)
		{
			OLED_ShowString(90,30,"+");
			OLED_ShowNumber(95,30,decoder_right,3,16);
		}else{
			OLED_ShowString(90,30,"-");
			OLED_ShowNumber(95,30,decoder_right,3,16);
		}
	
		OLED_ShowString(50,45," yaw:");
		if(yaw>=0)
		{
			OLED_ShowString(90,45,"+");
			OLED_ShowNumber(95,45,yaw,3,16);
		}else{
			OLED_ShowString(90,45,"-");
			OLED_ShowNumber(95,45,(-1)*yaw,3,16);				
		}
		
		OLED_ShowNumber(0,45,decoder_left,3,16);
